...
import { addImage } from '../api/addImage.js';
    ...
    if (page === 'admin-area') {
      ...
      const saveSubsubcategoryBtns = [
        ...document.getElementsByClassName(
          'admin_area_save_subsubcategory_btn',
        ),
      ];
      ...
      const editProductBtns = [
        ...document.getElementsByClassName('admin_area_product_card'),
      ];
      const saveMainImgInput = document.getElementsByClassName(
        'admin_area_save_main_img_input',
      )[0];
      const saveImgSetImgInput = document.getElementsByClassName(
        'admin_area_save_img_set_img_input',
      )[0];
      ...
      if (editProductBtns.length > 0) {
        editProductBtns.forEach(btn => {
          btn.addEventListener('click', () => {
            localStorage.setItem('editProduct', JSON.stringify(btn.id));
            location.reload();
          });
        });
      }
      ...
      if (saveMainImgInput) {
        saveMainImgInput.addEventListener('change', e => {
          const saveMainImgBtn = document.getElementsByClassName(
            'admin_area_save_main_img_btn',
          )[0];

          if (saveMainImgBtn) {
            saveMainImgBtn.addEventListener('click', async () => {
              const productId = saveMainImgInput.id;
              const main = true;
              const file = e.target.files[0];

              await addImage(productId, file, main);
              location.reload();
            });
          }
        });
      ...
      if (saveImgSetImgInput) {
        saveImgSetImgInput.addEventListener('change', e => {
          const saveImgSetImgBtn = document.getElementsByClassName(
            'admin_area_save_img_set_img_btn',
          )[0];

          if (saveImgSetImgBtn) {
            saveImgSetImgBtn.addEventListener('click', async () => {
              const productId = saveImgSetImgInput.id;
              const main = false;
              const file = e.target.files[0];
        ...
        [...favoritesList.children].forEach(child => {
          ...
          button.addEventListener('click', e => {
            ...
            if (favorites) {
              ...
              localStorage.setItem(
                'favorites',
                JSON.stringify(newFavoritesArr),
              );
              location.reload();
            }
          });
